...
const DetailsBook = ({ data }) => {
    ...
    // use selector
    const { allBookData, allBookDataOfFiltered } = useSelector(state => state.bookLoads)
...
`
    ...
    const removeBook = async (func) => {
        ...
        try {
            ...
            const dataToLoad = useDelete(allBookData, removeRef.current.id)
            const data = useDelete(allBookDataOfFiltered, removeRef.current.id)
            const reTodayData = useTodayData(data)
            const reTodayCount = useTotalBooks(reTodayData)

            // upadate data
            dispatch(bookLoadSuccess([dataToLoad, reTodayData, reTodayCount]))
